...
class _Meta(type):
    def __new__(cls: Type, name: str, bases: tuple[Type], cls_dict: dict):
        target_class = type.__new__(cls, name, bases, cls_dict)
        _register_class(target_class)
        return target_class


class Serializable(_SerializableBase, metaclass=_Meta):
    ...
